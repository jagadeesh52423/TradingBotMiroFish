...
class _Client:
    def current_price(self, s):
        return Decimal({"SBIN": "800", "RELIANCE": "2900", "TATAMOTORS": "1000"}[s])


def test_poll_emits_quote_events_for_all_symbols():
    got = []
    adapter = NubraFeedAdapter(_Client(), symbols=["SBIN", "RELIANCE", "TATAMOTORS"])
    adapter.register_callback(lambda ev: got.append(ev))
    adapter.subscribe(["SBIN", "RELIANCE", "TATAMOTORS"])
    adapter.poll_once()
